...
(()=>{
  ...
  const major=([name,route])=>`<a class="entry" href="/advaita/articles/scripture/${route}/"><span class="name">${esc(name)}</span><span class="meta">Purāṇa encyclopedia article · exact nine-section research template</span></a>`;
  const minor=([name,route])=>{const status=EXTANT.has(name)?'extant / transmitted':LOST.has(name)?'lost / fragmentary':'identity / preservation disputed';return `<a class="entry" href="/advaita/articles/scripture/upapuranas/${route}/"><span class="name">${esc(name)}</span><span class="meta"><span class="status">${status}</span>${count(name)} of 4 traditional list witnesses</span></a>`};
