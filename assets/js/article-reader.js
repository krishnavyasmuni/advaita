...
const figure=(file,alt,cls='')=>{const f=document.createElement('figure');f.className=`source-document-figure ${cls}`.trim();const img=document.createElement('img');img.src=`/vivekadrishti/assets/images/source-documents/${file}`;img.alt=alt;img.loading='lazy';img.decoding='async';f.append(img);return f};
