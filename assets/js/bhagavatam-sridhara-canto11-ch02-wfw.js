(() => {
  ...
  if (!host) return;
  const data = fetch('/vivekadrishti/assets/data/bhagavatam-sridhara-wfw-canto11-ch02-verses01-10.json?v=20260916-1', {cache: 'no-cache'})
    ...
    .catch(error => {console.warn('Śrīdhara 11.2 reviewed checkpoint unavailable', error); return [];});
  ...
  function applyEntry(entry) {
    ...
    if (!shell) return;
    const section = shell.querySelector('#sb-11-2-' + entry.start + (entry.end !== entry.start ? '-' + entry.end : ''));
    ...
    container.append(panel('Śrīdhara Sanskrit · 11.2.' + entry.start, entry.sanskrit, 'sb-bhasya', 'sa-Deva'));
